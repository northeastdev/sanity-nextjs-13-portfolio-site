...
export default function Navbar() {
  ...
  return (
    <>
      <header className="sticky py-5 top-0 z-[1] flex items-center justify-between text-white-primary backdrop-blur-xl 2xl:rounded-2xl">
        <nav className="px-4 md:px-6 lg:px-8 flex flex-wrap  items-center justify-between w-full">
          ...
          >
            <ul className="text-sm mt-7 md:mt-0 flex flex-col gap-y-6 md:flex-row md:items-center md:gap-x-8">
              {navigation.map((item) => (
                <li
                  className="nav-item text-black-secondary dark:text-white-primary"
                  key={item.id}
                >
                  <a
                    onClick={() => setNavbarOpen(!navbarOpen)}
